  ...
  <div class="log-console">
    ...
    <div class="filter-bar">
      <div class="filter-group">
        <span class="filter-label">Log levels:</span>
        ...
        <label class="filter-checkbox">
          ...
          <span class="filter-label-trace">Trace</span>
        </label>
      </div>
      <div class="filter-group">
        <span class="filter-label">Source file:</span>
    ...
    <div class="search-bar">
      <div class="search-input-wrapper">
        ...
        />
        <button v-if="searchKeyword" class="search-clear-btn" @click="clearSearch" title="Clear search">
          <el-icon><Close /></el-icon>
        </button>
      </div>
      <div v-if="searchKeyword" class="search-info">
        <span class="search-info-icon">🔍</span>
        Found <span class="search-info-count">{{ filteredLogs.length }}</span> match(es)
      </div>
    </div>
    ...
    <div class="log-container" ref="logContainer">
      <div v-if="filteredLogs.length === 0" class="empty-state">
        ...
        <div class="empty-state-text">
          {{ loading ? 'Loading logs...' : searchKeyword ? 'No matching logs found~' : 'No logs recorded yet~' }}
        </div>
      </div>
...
async function clearLogs() {
  if (await confirm('Clear all logs?')) {
    try {
